...
async function showMsg() {
    ...
    if (typeof $.JDtotalcash !== "undefined") {
        ReturnMessage += `极速金币：${$.JDtotalcash}金币(≈${
            $.JDtotalcash / 10000
        }元)\n`;
    }
    if (typeof $.JdzzNum !== "undefined") {
        ReturnMessage += `京东赚赚：${$.JdzzNum}金币(≈${
            $.JdzzNum / 10000
        }元)\n`;
    }
    if ($.JdMsScore != 0) {
        ReturnMessage += `京东秒杀：${$.JdMsScore}秒秒币(≈${
            $.JdMsScore / 1000
        }元)\n`;
    }
    if ($.JdFarmProdName != "") {
        if ($.JdtreeEnergy != 0) {
            ReturnMessage += `东东农场：${$.JdFarmProdName},进度${(
    ...
    if (
        ...
        if (response.resultCode === "0") {
            ...
            ReturnMessage += `勋章${response.result.medalNum}/${
                response.result.medalNum + response.result.needCollectMedalNum
            }块(${response.result.medalPercent}%)\n`;
                ...
                if (err) {
                    ...
                    if (data) {
                        ...
                        if (
                            ...
                            $.beanCount =
                                data.data && data.data["assetInfo"]["beanNum"];
                        }
                    } else {
            ...
            try {
                if (err) {
                    ...
                    if (data) {
                        ...
                        $.message += `当前总红包：${$.balance}(今日总过期${
                            $.expiredBalance
                        })元 🧧\n京喜红包：${
                            $.jxRed
                        }(今日将过期${$.jxRedExpire.toFixed(
                            2
                        )})元 🧧\n极速红包：${
                            $.jsRed
                        }(今日将过期${$.jsRedExpire.toFixed(
                            2
                        )})元 🧧\n京东红包：${
                            $.jdRed
                        }(今日将过期${$.jdRedExpire.toFixed(
                            2
                        )})元 🧧\n健康红包：${
                            $.jdhRed
...
function getMs() {
    return new Promise((resolve) => {
        $.post(
            taskMsPostUrl("homePageV2", {}, "appid=SecKill2020"),
            (err, resp, data) => {
                try {
                    if (err) {
                        console.log(
                            `${err},${jsonParse(resp.body)["message"]}`
                        );
                        console.log(`${$.name} API请求失败，请检查网路重试`);
                    } else {
                        if (safeGet(data)) {
                            ...
                            if (data.code === 2041) {
                                $.JdMsScore =
                                    data.result.assignment.assignmentPoints ||
                                    0;
                            }
                        }
                    }
...
function taskMsPostUrl(function_id, body = {}, extra = "", function_id2) {
    ...
    return {
        ...
        body: `functionId=${function_id}&body=${escape(
            JSON.stringify(body)
        )}&client=wh5&clientVersion=1.0.0&${extra}`,
        headers: {
            Cookie: cookie,
                    ...
                    if (safeGet(data)) {
                        ...
                        if ($.farmInfo.farmUserPro) {
                            ...
                            let waterTotalT =
                                ...
                                10; //一共还需浇多少次水
                            let waterD = Math.ceil(
                                waterTotalT / waterEveryDayT
                            );
                ...
                try {
                    if (err) {
                        ...
                        if (safeGet(data)) {
                            data = JSON.parse(data);
                            $.JDwaterEveryDayT =
                                data.totalWaterTaskInit.totalWaterTaskTimes;
                        }
                    }
                } catch (e) {
                    $.logErr(e, resp);
...
function taskcashUrl(_0x7683x2, _0x7683x3 = {}) {
    ...
    let _0x7683x5 = `${__Oxb24bc[0x0]}${JSON[__Oxb24bc[0x1]](_0x7683x3)}${
        __Oxb24bc[0x2]
    }${_0x7683x2}${__Oxb24bc[0x3]}${_0x7683x4}${__Oxb24bc[0x4]}`;
    ...
    return {
        url: `${__Oxb24bc[0x8]}${JD_API_HOST}${__Oxb24bc[0x9]}${_0x7683x2}${
            __Oxb24bc[0xa]
        }${escape(JSON[__Oxb24bc[0x1]](_0x7683x3))}${
            __Oxb24bc[0xb]
        }${_0x7683x4}${__Oxb24bc[0xc]}${_0x7683x8}${__Oxb24bc[0x8]}`,
        headers: {
            "\x48\x6F\x73\x74": __Oxb24bc[0xd],
            "\x61\x63\x63\x65\x70\x74": __Oxb24bc[0xe],
            "\x6B\x65\x72\x6E\x65\x6C\x70\x6C\x61\x74\x66\x6F\x72\x6D":
                __Oxb24bc[0xf],
...
(function (_0x7683x9, _0x7683xa, _0x7683xb, _0x7683xc, _0x7683xd, _0x7683xe) {
    ...
    _0x7683xd = _0x7683xb(
        __Oxb24bc[0x14],
        _0x7683xb(_0x7683xb(__Oxb24bc[0x15], __Oxb24bc[0x16]), __Oxb24bc[0x17])
    );
    try {
        _0x7683x9 = __encode;
        if (
            !(
                typeof _0x7683x9 !== _0x7683xe &&
                _0x7683x9 === _0x7683xb(__Oxb24bc[0x18], __Oxb24bc[0x19])
            )
        ) {
            _0x7683xc(_0x7683xd);
        }
        ...
        $.get(myRequest, (err, resp, data) => {
            try {
                if (err) {
                    ...
                    data = JSON.parse(
                        data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1]
                    );
                    if (data.ret === 0) {
...
function decrypt(time, stk, type, url) {
    ...
    if (stk) {
        ...
        if ($.fingerprint && $.Jxmctoken && $.enCryptMethodJD) {
            ...
            hash1 = $.CryptoJS.SHA512(str, $.Jxmctoken).toString(
                $.CryptoJS.enc.Hex
            );
        }
        let st = "";
        ...
        $.post(options, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`);
                    console.log(
                        `request_algo 签名参数API请求失败，请检查网路重试`
                    );
                } else {
                    if (data) {
                        data = JSON.parse(data);

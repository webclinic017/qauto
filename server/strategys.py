...
class FixedInvestStrategy(bt.Strategy):
    ...
    def start(self):
        ...
        if self.params.doprint:
            ...
            df = self.db.select(self.dbname, wheres)
            # 策略已经执行
            if not df.empty:
                msg = '策略已经执行'
                print(msg)
                self.params.optpass = True
                self.stop()
    ...
    def stop(self):
        if self.params.doprint:
            ...
            df = pd.DataFrame([optresult])
            self.db.update(df, self.dbname)
            print(msg)
